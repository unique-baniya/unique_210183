...
def button(frame):          #Function to define a button
    b=Button(frame,padx=1,bg="papaya whip",width=2,text="   ",font=('arial',40,''),relief="sunken",bd=20)
    return b
...
def reset():         # this funciton will reset the game
    global a
    for i in range(3):
        for j in range(3):
                b[i][j]["text"]=" "
...
def check():                #it will check who will be winner
    for i in range(3):
            if(b[i][0]["text"]==b[i][1]["text"]==b[i][2]["text"]==a or b[0][i]["text"]==b[1][i]["text"]==b[2][i]["text"]==a):
                    messagebox.showinfo("Congratulation!!","'"+a+"' has won")
                    reset()
...
root.title("Tic-Tac-Toe")   # title is given
a=r.choice(['O','X'])       # two players are defined
